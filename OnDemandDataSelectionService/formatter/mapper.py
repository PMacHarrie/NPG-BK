...
def main(argv):
	...
	psf = open(workDir + '/dss.pl.PSF')

	fileName=psf.readline().rstrip()
	print fileName
	nc4File = workDir + '/' + fileName
	...
	print "f=", f
	Measurement = f.variables['Radiance@VIIRS-I4-SDR'][::5,::5]

	fig=plt.figure(3, figsize=(18,15))

	#ax1 = fig.add_axes([1.0,2.0,3.0,4.0])

	lats=f.variables['Latitude@VIIRS-IMG-GEO-TC'][::5,::5]
	minLat = np.amin(lats)
	maxLat = np.amax(lats)
	lons=f.variables['Longitude@VIIRS-IMG-GEO-TC'][::5,::5]
	minLon = np.amin(lons)
	maxLon = np.amax(lons)

	print "minLat=", minLat, minLon, maxLat, maxLon

#minLat= 19.0725 -48.8821 38.3759 -13.6537

	fig=plt.figure(3, figsize=(18,15))

#	m=Basemap(width=1800000, height=1200000, 
	m=Basemap(
		projection='merc',
		llcrnrlon=minLon,
		llcrnrlat=minLat,
...
#		lat_0=29.0)
	...
	#m.bluemarble()
	#m.drawstates(color='lightskyblue', zorder=20)

	Measurement=np.ma.masked_array(Measurement, Measurement <= 0.)
	xx,yy = m(lons,lats)
	m.pcolormesh(xx,yy,Measurement, cmap=cm.GMT_haxby, alpha=0.6)

#variables(dimensions): float32 Latitude@VIIRS-IMG-GEO-TC(AlongTrack-4608,CrossTrack-6400), float32 Longitude@VIIRS-IMG-GEO-TC(AlongTrack-4608,CrossTrack-6400), uint16 Radiance@VIIRS-I4-SDR(AlongTrack-4608,CrossTrack-6400)

	plt.savefig('test.png', transparent=True, transparency=.6)
	plt.show()
        exit()
