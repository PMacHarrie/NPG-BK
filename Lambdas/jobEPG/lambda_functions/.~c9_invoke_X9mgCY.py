    ...
    if inResult is None:
        ...
        if inResult.get('sort') is not None:
            ...
            for sortItem in inResult.get('sort'):
                ...
                if attrName == 'algoReturnCode':
                    attrName = 'odalgorithmreturncode'
                elif attrName == 'dssReturnCode':
                    attrName = 'oddataselectionreturncode'
                elif attrName == 'jobCpuUtil':
                    attrName = 'odjobcpu_util'
                elif attrName == 'jobMemUtil':
                    attrName = 'odjobmem_util'
                elif attrName == 'jobIoUtil':
                    attrName = 'odjobio_util'
                elif not attrName.lower().startswith('odjob'):
        ...
        for row in rows:
            resultArray.append( 
                {"jobId" : row[0], 
                 ...
                 "jobStatus": row[3],
                 "enqueueTime": row[4].strftime("%Y-%m-%dT%H:%M:%S") if row[4] is not None else "",
                 "startTime": row[5].strftime("%Y-%m-%dT%H:%M:%S") if row[5] is not None else "",
                 "completionTime": row[6].strftime("%Y-%m-%dT%H:%M:%S") if row[6] is not None else ""
                } )
...
def parseTimeRange(timeInputVal, timeParamName, timeColumnName, whereConds, queryParams):
    ...
    try:
        ...
        raise ValueError("lte and gte must be formatted as: YYYYMMDDTHHmmSS.sssZ")
