...
def lambda_handler(event, context):
    ...
    if inQuery.get('jobStatus') is not None:
        ...
        queryParams['jobStatus'] = inQuery['jobStatus']
    ...
    if algo is not None:
        ...
        whereClauses.append(" odalgorithmname in %(algorithmName)s ")
        whereClauses.append(" odalgorithmversion in %(algorithmVersion)s ")
    ...
    if inResult is None:
        ...
        if inResult.get('sort') is not None:
            ...
            for sortItem in inResult.get('sort'):
                ...
                if not re.match('[a-zA-Z0-9]', attrName):
    ...
    # "SELECT odjobid, odalgorithmname, odalgorithmversion, odjobstatus, odjobenqueuetime, " + 
    #     "odjobstarttime, odjobcompletiontime FROM ondemandjob WHERE 
