...
def lambda_handler(event, context):
    ...
    try:
        ...
        query = cur.mogrify("SELECT odjobid, odalgorithmname, odalgorithmversion, odjobstatus, odjobenqueuetime, " + 
            "odjobstarttime, odjobcompletiontime, odjoboutput FROM ondemandjob WHERE " + ' AND '.join(whereConds) +
        ...
        for row in rows:
            resultArray.append( 
                {"jobId" : row[0], 
                 ...
                 "jobStatus": row[3],
                 "jobEnqueueTime": row[4].strftime("%Y-%m-%dT%H:%M:%S.%fZ") if row[4] is not None else "",
                 "jobStartTime": row[5].strftime("%Y-%m-%dT%H:%M:%S.%fZ") if row[5] is not None else "",
                 "jobCompletionTime": row[6].strftime("%Y-%m-%dT%H:%M:%S.%fZ") if row[6] is not None else "",
                 "jobOutput": row[7]
                } )
