    ...
    try:
        ...
        try:
            if executableName.endswith("py"):
                ...
                spCmd = ["python3", "./lambda_functions/extractors/" + executableName]
            elif executableName.endswith("pl"):
                print(os.getcwd())
                print(os.listdir('./lambda_functions/extractors/'))
                spCmd = ["perl", "./lambda_functions/extractors/" + executableName]
            else:
                raise Exception("Only Python or Perl executable supported: " + executableName)
            
            if optionalParameter is not None and len(optionalParameter) > 0  \
            ...
            out = subprocess.check_output(spCmd, stderr=subprocess.STDOUT)
            
        except subprocess.CalledProcessError as cpe:
            s = "EME failed with rc: " + str(cpe.returncode) + ", output: " + str(cpe.output.decode())
            print(s)
