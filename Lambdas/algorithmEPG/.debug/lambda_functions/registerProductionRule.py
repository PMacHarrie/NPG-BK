...
def lambda_handler(event, context):
    ...
    try:
        ...
        try:
            doPrOutputs(conn, prRule.get('ProductionRuleOutputs'), prRuleId, isUpdate, algoId)
        except ValueError as e:
            print("Exception in doPrOutputs:", e)
            return createErrorResponse(400, "Validation error", str(e))
        
        # Convert prStartBoundaryTime to str for json.dumps
        if prRule.get('prStartBoundaryTime') is not None:
            prRule['prStartBoundaryTime'] = prRule.get('prStartBoundaryTime').strftime("%Y-%m-%d %H:%M:%S.%f")
        esSuccess = createElasticDoc(prRuleId, json.dumps({"ProductionRule": prRule}))
        ...
        if esSuccess:
            ...
            return createErrorResponse(500, "Internal Error", "Error encountered updating ES")
...
def doPrRule(conn, prRule, prRuleId, isUpdate, algoId):
    ...
    if isStrEmptyNull(prRule.get('prStartBoundaryTime')):
        ...
        dtFormatSuccess = False
        for dtFormat in ['%Y-%m-%d %H.%M.%S.%f','%Y-%m-%d %H:%M:%S.%f']:
            try:
                sbt = datetime.strptime(prRule.get('prStartBoundaryTime'), dtFormat)
                dtFormatSuccess = True
            except Exception as e:
                print(str(e))
                print("Format", dtFormat, "failed for", prRule.get('prStartBoundaryTime'), "...trying another")
        if not dtFormatSuccess:
            raise ValueError("Unable to format prStartBoundaryTime, " +
                    "should be '%Y-%m-%d %H.%M.%S.%f  OR  %Y-%m-%d %H:%M:%S.%f")
...
def doPrInputs(conn, prInputs, prRuleId, isUpdate, algoId):
    ...
    for prInput in prInputs:
        ...
        for inputProd in prInput.get('PRInputProducts'):
            ...
            if inputProd.get('platformName') == 'N/A':
                prodPlatStr = inputProd.get('productShortName')
                cur.execute("SELECT pd.productid FROM productdescription pd, algoinputprod aip " + 
                    "WHERE pd.productid = aip.productid AND aip.algorithmid = %s AND productshortname = %s", 
                    (algoId, inputProd.get('productShortName')))
                row = cur.fetchone()
                if row is None:
                    raise ValueError("Product has not been registered to the algorithm or product is invalid:" + 
                        inputProd.get('productShortName') )
                else:
                    productId = row[0]
            else:
                prodPlatStr = inputProd.get('productShortName') + "/" + inputProd.get('platformName')
            ...
            if isUpdate:
                ...
                print("Added prInputProduct:", prodPlatStr, "pripId:", str(pripId))
    
    cur.close()
    print(">> Finished doPrInputs")
    
    
def doPrOutputs(conn, prOutputs, prRuleId, isUpdate, algoId):
    ...
    prosIds = ((),)
    if isUpdate:
        cur.execute("SELECT prosid FROM proutputspec WHERE prid = %s ", (prRuleId, ))
        prosIds = cur.fetchall()
        if len(prosIds) != len(prOutputs):
            raise ValueError("ProductionRuleOutputs count does not match existing count (" + len(prosIds) + ")")
        
    platforms = {}
    for idx, outputProd in enumerate(prOutputs):
        if outputProd.get('platformName') is None:
            raise ValueError("Missing platformName in ProductionRuleOutputs")
            
        if outputProd.get('platformName') == 'N/A':
            prodPlatStr = outputProd.get('productShortName')
            cur.execute("SELECT pd.productid FROM productdescription pd, algooutputprod aop " + 
                    "WHERE pd.productid = aop.productid AND aop.algorithmid = %s AND productshortname = %s", 
                    (algoId, outputProd.get('productShortName')))
            row = cur.fetchone()
            if row is None:
                raise ValueError("Product has not been registered to the algorithm or product is invalid:" + 
            ...
            prodPlatStr = outputProd.get('productShortName') + "/" + outputProd.get('platformName')
            ...
            if row is None:
                ...
                productId = row[0]
        if isUpdate:
            cur.execute("UPDATE proutputspec SET productid = %s WHERE prosid = %s", (productId, prosIds[idx]))
            print("Updated prosId", str(prosIds[idx]), "to", prodPlatStr)
        else:
            ...
            print("Added prOutputProduct:", prodPlatStr, "prosid:", str(prosId))
...
def createElasticDoc(prRuleId, prRuleJson):
    ...
    try:
        ...
        if res['result'] == 'created' and res['_id'] == str(prRuleId):
            ...
            return True
        elif res['result'] == 'updated' and res['_id'] == str(prRuleId):
