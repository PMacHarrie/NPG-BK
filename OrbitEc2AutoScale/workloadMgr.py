...
def get_ObsPlan(platformName, site):
	...
	sql = """
		select 
			Null,
			to_char(min(aos_time), 'MM-DD-YY HH24:MI:SS'),
			to_char(min(aos_time) + interval '59' minute, 'MM-DD-YY HH24:MI:SS'),
			extract(epoch from min(aos_time) - now())
		from observatorydataplan
		where (
		        (platformName = 'SNPP') or (platformName = 'J1' and site = 'McMurdo')
		        )
	...
	sql = "select min(to_char(aos_time, 'MM-DD-YY HH24:MI:SS')) from observatorydataplan where ((platformName = 'SNPP') or (platformName = 'J1' and site = 'McMurdo')) and aos_time >= now()"
	cursor.execute(sql, (platformName,site))
	row = cursor.fetchone()
...
sql = """
	select min(aos_time), count(*) from observatorydataplan where platformname = 'SNPP' and aos_time >= now()
	"""
...
while loopy:
	for asgName in resources["autoscaleGroups"]:
		...
		obsPlan = get_ObsPlan('SNPP', 'Svalbard')
