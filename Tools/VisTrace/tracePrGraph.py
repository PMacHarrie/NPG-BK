...
def getJobDetails(prId, cur, nodes, edges, visitedRuleIds):
    ...
    query = "SELECT distinct pr.prId FROM productionrule pr, proutputspec pros " + \
        " WHERE pr.prid = pros.prid and pros.productid in (%s)" % ','.join(str(x) for x in inProdIds) + \
        " UNION SELECT distinct pr.prId FROM productionrule pr,  prinputspec pris, prinputproduct prip " + \
