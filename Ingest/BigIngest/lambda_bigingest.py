...
from extractors import edm_idpsH5  as idpsExtr
from extractors import edm_nupNc4  as nupExtr
from extractors import edm_goesCgs as cgsExtr
from extractors import edm_eme     as emeExtr
...
def persistIrTimings(status, failureReason):
    ...
    cur.execute("INSERT INTO ingestlog (il_id, productid, fileid, filename, ilstatus, ilfailurereason, ilmetrics_json) " +
            ...
            (productId, fileId, ingestReqObj['fileName'], "BIG " + status, failureReason, json.dumps(irTimings['log'])) )
    conn.commit()
    cur.close()
