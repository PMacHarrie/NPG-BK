...
insQueueUrl = 'https://sqs.us-east-1.amazonaws.com/784330347242/GOESRBigData'
...
def pollQueueAndProcess(conn):
    ...
    if 'Messages' in response:
        ...
        record = msg.get('Records')[0]
        
        goesKey = record.get('s3').get('object').get('key')
        goesBucket = record.get('s3').get('bucket').get('name')
        filename = goesKey.split('/')[-1]

        logger.info("Got message (sentTs {0}) filename: {1}".format(sentTs, filename) )

# Jonathan Hansford 3/5/2019 - added filter to accept only ABI-L1b-Rad files (to deny CMIP files).
#        goesProductName = goesKey.split('/')[0] # This is the GOES-16 bucket's name for the product.
...
#            )
#            logger.info("deleted sqs message")
    # End of if 'Messages' in response
    else:
